...
Login.prototype.middleware = function(store, options) {
  ...
  var defaultOptions = {
    ...
    passport: {
      ...
      registerCallback: function (req, res, user, done) { done() }
    },
    verifyCallback: function (user, profile, done) { done(null, user) },
    registerCallback: function (user, profile, done) { done(null, user) },
    strategies: {},
    user: {
      id: true,
      timestamps: true
