...
function registerProvider($user, provider, profile, req, res, done) {
  ...
  /*} if (options.localFirst && provider !== 'local') {
    ...
    options.registerCallback(user, profile, function(err, user) {
      if (err || !user) done(err, user);
      
      model.add(options.collection, user, function(err) {
        if (err) return done(err);
  
        user = model.get(options.collection + '.' + user.id);
        // Legacy registerCallback
        options.passport.registerCallback(req, res, user, function () {
          login(user, req, done);
        });
      });
    });
